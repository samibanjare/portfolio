...
// Passive thoughts shared while idle
const IDLE_THOUGHTS = [
  "Optimizing my gradient descent vectors... 🧠",
  "Did someone clear the terminal history? 🧐",
  "System status: 100% stable. Feeling smart today.",
  "I wonder if P vs NP will ever be solved... ⏳",
  "RL agent reward function maximized! 🏆",
  "Just thinking about clean code architectures. 💻",
  "My synaptic fire rate is humming nicely. ⚡"
];

// Contextual remarks triggered on cursor movement mapping to specific expressions
const INTERACTIVE_REMARKS = {
  HAPPY: [
  ...
  SURPRISED: [
    ...
    "Input detected! Processing... 📡",
    "Ah! A wild visitor appears!"
  ],
  SASSY: [
    "Are you just going to hover there all day? 😏",
    "Check out my terminal, it's pretty slick. ✨",
    "My loss function is lower than yours."
  ],
  ANNOYED: [
    "Hey, watch the cursor! 💢",
    "Don't make me spin up an infinite while loop... 🤖",
    "Warning: High latency detected from your mouse clicks."
  ]
};
...
export default function RobotPet() {
  ...
  const [speech, setSpeech] = useState("Hello! I am Nexus. Type 'help' in the terminal!");
  
  // Ref to track the last time an expression was changed
  const lastChangeTime = useRef(0);

  // Passive automatic blinking state loop and thought generator when mouse is away
  useEffect(() => {
    const interval = setInterval(() => {
      setExpression((prev) => {
        if (prev === "NORMAL") {
          ...
          // Smoothly update to a fresh idle thought on every auto-blink
  ...
  const triggerRandomExpression = () => {
    ...
    // ⏱️ Lockout Delay: Only allow a change every 500ms for conversational readability
    ...
    // Assign corresponding remark text based on the random choice context
    if (randomChoice === "NORMAL") {
      setSpeech("Just hanging out on your screen. 😎");
    } else {
      const remarksPool = INTERACTIVE_REMARKS[randomChoice];
      const randomRemark = remarksPool[Math.floor(Math.random() * remarksPool.length)];
      setSpeech(randomRemark);
    }
  ...
  const handleMouseLeave = () => {
    ...
    setSpeech("Goodbye for now! I'll be right here waiting.");
  };
    ...
    <div style={{ textAlign: "center", padding: "10px", userSelect: "none" }}>
      ...
      <div 
        ...
        style={{ 
          ...
          alignItems: "center",
          cursor: "pointer",
          paddingTop: "65px" // Creates static clearance space for the speech bubble overlay
        }}
      >
        ...
        <div style={{
          position: "absolute",
          top: "53px", // Adjusted to lock down perfectly on top of your static head positioning matrix
          width: "164px",
          height: "44px",
        ...
        <div style={{
          position: "absolute",
          left: "-1px",
          top: "99px", // Locked step-for-step with your structural earpieces coordinates
          width: "28px",
          height: "44px",
